...
class LoadBasicData implements FixtureInterface
{
	...
	/**
	 * Load Basic Data to make the project work
	 * - in cmd : php app/console doctrine:schema:drop --force; php app/console doctrine:schema:create; php app/console doctrine:fixtures:load
	 */
    public function load(ObjectManager $manager)
    {
		...
		// PERMISSIONS
			...
			$hide_list = new Role();
			$hide_list->setName('ROLE_HIDE_LIST');
			$manager->persist($hide_list);

			$hide_create = new Role();
			...
			$manager->persist($hide_user);

			$hide_report = new Role();
			$hide_report->setName('ROLE_HIDE_REPORT');
			$manager->persist($hide_report);

			$hide_order = new Role();
			$hide_order->setName('ROLE_HIDE_ORDER');
			$manager->persist($hide_order);

			$hide_popup = new Role();
			$hide_popup->setName('ROLE_HIDE_POPUP');
			$manager->persist($hide_popup);

			$hide_ticket = new Role();
			$hide_ticket->setName('ROLE_HIDE_TICKET');
			$manager->persist($hide_ticket);

			$hide_19 = new Role();
			$hide_19->setName('ROLE_HIDE_19');
			$manager->persist($hide_19);

			$hide_20 = new Role();
			$hide_20->setName('ROLE_HIDE_20');
			$manager->persist($hide_20);

		// SERVICES

			$serv_ged = new Role();
			$serv_ged->setName('ROLE_SERV_GED');
			$manager->persist($serv_ged);

			$serv_adservice_es = new Role();
			$serv_adservice_es->setName('ROLE_SERV_ADSERVICE_ES');
			$manager->persist($serv_adservice_es);

			$serv_adservice_pt = new Role();
			$serv_adservice_pt->setName('ROLE_SERV_ADSERVICE_PT');
			$manager->persist($serv_adservice_pt);

			$serv_assistance_diag_24 = new Role();
			$serv_assistance_diag_24->setName('ROLE_SERV_ASSISTANCE_DIAG_24');
			$manager->persist($serv_assistance_diag_24);

			$serv_phone_eina_ecp = new Role();
			$serv_phone_eina_ecp->setName('ROLE_SERV_PHONE_EINA_ECP');
			$manager->persist($serv_phone_eina_ecp);

			$serv_phone_eina_js = new Role();
			$serv_phone_eina_js->setName('ROLE_SERV_PHONE_EINA_JS');
			$manager->persist($serv_phone_eina_js);

			$serv_phone_eina_technodiag = new Role();
			$serv_phone_eina_technodiag->setName('ROLE_SERV_PHONE_EINA_TECHNODIAG');
			$manager->persist($serv_phone_eina_technodiag);

			$serv_actia = new Role();
			$serv_actia->setName('ROLE_SERV_ACTIA');
			$manager->persist($serv_actia);

			$serv_nexus = new Role();
			$serv_nexus->setName('ROLE_SERV_NEXUS');
